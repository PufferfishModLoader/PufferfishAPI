...
public class NetHandlerPlayClientTransformer implements ClassTransformer {
    ...
    public ClassNode transformClass(ClassNode classNode) {
        for (MethodNode methodNode : classNode.methods) {
            if (methodNode.name.equals("handleJoinGame")) {
                ...
                list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, TransformerUtils.getImplementationClass("NetHandlerPlayClient"), "handleJoinGame", "(ZLjava/net/SocketAddress;)V"));

                methodNode.instructions.insert(list);
            } else if (methodNode.name.equals("onDisconnect")) {
                InsnList list = new InsnList();
                list.add(new VarInsnNode(Opcodes.ALOAD, 1));
                list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, TransformerUtils.getImplementationClass("NetHandlerPlayClient"), "handleDisconnect", "(Lnet/minecraft/util/text/ITextComponent;)V"));
